%TODO titel ändern
\subsection{State Probabilities}\label{sec:cloud:data_centers:closed_form_solution}
Using the macro state equations discussed in \refsec{sec:cloud:data_centers:modeling}, multiple ways to obtain the state probabilities are possible.
...
All equations feature a factor \(x(0, 0)\) which in turn can be calculated using the normalization property given in \refeq{eq:cloud:data_centers:modeling:normative}.
...
\end{align*}
Using this definition, we get the state probability for \(\theta_1\) jobs in the system with activated reserved servers:
\begin{align*}
...
Applying this redefinition to \refeq{eq:probability_greater_n+m} we get
\begin{align*}
x(i>n+m,1) &= \sum_{i = n + m + 1}^{+ \infty} x(i, 1)\\
