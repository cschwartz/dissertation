...
Afterwards, we observe an increase in saved connection time for larger thresholds, because here files are sent in batches and the mobile client disconnects between the sending intervals, again enabling power saving potential for the mobile network client.
...
In the case of the \algointerval algorithm, the amount of uploaded data varies, because of the different image sizes.
...
While the user wants to minimize the mean waiting time of the file synchronization, he want to maximize the mean relative disconnected time to save power.
